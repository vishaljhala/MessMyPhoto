...
extern "C" __declspec(dllexport) void AbstractLineDrawing(char *szPath, int bPreviewMode, int*iErrCode)
{
	...
	clsAbstractLineDrawing gs;
	gs.AbstractLineDrawingMain(&pImgSource,iErrCode,g_GH);
	...
	*iErrCode = 0;
	...
	sprintf(pszTimec,"AbstractLineDrawing,%d-%02d-%02d %02d:%02d:%02d,%ld\0",stStart.wYear,stStart.wMonth,stStart.wDay,stStart.wHour,
...
clsAbstractLineDrawing::clsAbstractLineDrawing()
{
}
clsAbstractLineDrawing::~clsAbstractLineDrawing()
{
}
void clsAbstractLineDrawing::AbstractLineDrawingMain(IplImage **pImgSrc,int*iErrCode,GlobalHelper &g_GH)
{
	pImgSource = *pImgSrc;
...
void clsAbstractLineDrawing::GreyScale(GlobalHelper &g_GH)
{
	int iL,iU,iV;
	int iWStep, iJStep;
	int tmp;
